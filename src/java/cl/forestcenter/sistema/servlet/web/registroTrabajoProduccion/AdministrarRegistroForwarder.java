 ...
 * @author jjara
 */
public class AdministrarRegistroForwarder extends HttpServlet {
        ...
        if (cLogin.validarSesion(req.getSession().getAttribute("idUsuario").toString(), req.getSession().getId())) {
            ...
            if (req.getParameter("accion") != null && !req.getParameter("accion").equals("")) {
                if (req.getParameter("accion").equals("LISTADO")) {
                    listarRegistroForwarder(req, res);
                } else if (req.getParameter("accion").equals("LISTADO_ALL_REGISTROS")) {
                    listarAllRegistroForwarder(req, res);
                }
            } else {
                pagina = "/web/registroTrabajoProduccion/forwarder/administrarRegistroForwarder.jsp";
    ...
    private void listarRegistroForwarder(HttpServletRequest req, HttpServletResponse res) {
        int fila = 1;
        try {
        ...
        //mas codigo
        lista = cRegistroForwarder.getRegistroForwarder(page, rows, where);
        
        
        //FIN CODIGO VARIABLE
        int size = cRegistroForwarder.cantidadRegistrosForwarder("");
    ...
    private void listarAllRegistroForwarder(HttpServletRequest req, HttpServletResponse res) {
        //CODIGO VARIABLE
        ControladorRegistroForwarder cRegistroForwarder = new ControladorRegistroForwarder();
        List<RegistroForwarderDTO> lista = new ArrayList<RegistroForwarderDTO>();
